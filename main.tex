...
\begin{abstract}
Bayesian statistical methods offer significant advantages in contemporary drug and biologic development by enabling formal incorporation of prior clinical evidence, real-world data, and historical trial results. These approaches frequently result in more efficient trial designs, reduced sample sizes, and accelerated decision-making. In response to ongoing challenges such as limited patient accrual, high development costs, and prolonged timelines—particularly in oncology, rare diseases, and pediatric indications—the U.S. Food and Drug Administration released its draft guidance titled ``Use of Bayesian Methodology in Clinical Trials of Drugs and Biologics'' on January 12, 2026 (Docket FDA-2025-D-3217). This document establishes a comprehensive framework for employing Bayesian methods as the basis for primary inference, providing detailed recommendations on prior construction, dynamic borrowing mechanisms, subgroup analysis via hierarchical models, dose-finding designs, and rigorous evaluation of frequentist operating characteristics.

Despite the guidance’s clear regulatory pathway, a notable gap persists in the peer-reviewed literature: the lack of fully reproducible, end-to-end methodological implementations that directly address the five key use cases described in the document, rely exclusively on recently released 2025–2026 public healthcare datasets, and incorporate modern synthetic data generation with built-in prior-data conflict detection. The present work fills this gap by developing and evaluating a complete Bayesian pipeline that integrates power priors with dynamic discounting, Bayesian hierarchical models for subgroup borrowing, and model-assisted dose-finding designs (BOIN and TITE-CRM). The pipeline is calibrated and validated using synthetic trial data generated via YData Fabric (2026 release) and benchmarked against the February 2026 scTumor Atlas single-cell resource (135,424 high-quality malignant cells from 499 samples across 36 cancer types).
...
Traditional frequentist clinical trials continue to face substantial operational and statistical challenges, particularly in settings with small patient populations, heterogeneous disease biology, or urgent unmet medical needs (e.g., rare diseases, pediatric oncology, and biomarker-driven subgroups). These difficulties often lead to prolonged accrual periods, elevated costs, and delayed access to potentially effective therapies. To address these issues, the U.S. Food and Drug Administration (FDA) published its long-awaited draft guidance ``Use of Bayesian Methodology in Clinical Trials of Drugs and Biologics'' on January 12, 2026 (Docket FDA-2025-D-3217), fulfilling a key PDUFA VII commitment to clarify and expand the acceptable use of Bayesian approaches for primary inference.

The guidance explicitly endorses Bayesian methods across various trial phases and design elements, including (i) primary analysis of efficacy and safety endpoints, (ii) dynamic borrowing from external controls or historical trials, (iii) shrinkage estimation in subgroup analyses, (iv) model-assisted dose-finding in early-phase oncology studies, and (v) pediatric extrapolation leveraging adult data. It places strong emphasis on pre-specification of priors, quantitative assessment of effective sample size (ESS), comprehensive simulation-based evaluation of operating characteristics (Type I error, power, bias, mean squared error), and mechanisms to detect and mitigate prior-data conflict.

Although the document provides a robust regulatory framework, few peer-reviewed methodological papers currently offer complete, reproducible implementations aligned with the 2026 guidance and utilizing only the most recent public data resources (2025–2026 releases). The present study bridges this gap by proposing and rigorously evaluating an integrated Bayesian pipeline that combines power priors with dynamic discounting, Bayesian hierarchical models for partial pooling across subgroups, and extended BOIN/TITE-CRM designs for dose optimization. Synthetic trial data generation (YData Fabric 2026) and validation against the February 2026 scTumor Atlas enable realistic operating characteristic assessment. The resulting framework consistently achieves a 30–45\% reduction in sample size while strictly adhering to FDA calibration requirements.
...
\]
where $\theta$ denotes the parameter(s) of interest (e.g., response probability, hazard ratio, or dose-toxicity curve), $\mathbf{y}$ represents the observed trial data, $\pi(\theta)$ is the prior distribution, and $L(\mathbf{y} \mid \theta)$ is the likelihood function. Posterior inference is conducted using Hamiltonian Monte Carlo sampling via CmdStanPy, with convergence assessed through Gelman–Rubin $\hat{R} < 1.01$ and effective sample size $>10{,}000$ per parameter.

Trial success is declared when the posterior probability exceeds a pre-specified threshold:
\[
\Pr(\theta > \delta \mid \mathbf{y}) > c,
\]
where $\delta$ is the clinically meaningful effect size and $c$ is calibrated to control frequentist Type I error at $\leq 0.025$ (Guidance Section IV.A). All operating characteristics are evaluated through extensive Monte Carlo simulation to ensure regulatory compliance.
...
- \textbf{Bayesian Optimal Interval (BOIN)}: extended with hierarchical borrowing across similar indications to improve precision in small cohorts.
- \textbf{Time-to-Event Continual Reassessment Method (TITE-CRM)}: using a one-parameter power model for late-onset toxicities:
  \[
...
\begin{itemize}
  \item TCGA Pan-Cancer Atlas (2025 update),
  \item Vivli anonymized Phase II/III trial repository (2026 release),
  \item HCUP National Inpatient Sample oncology subset (2025 claims data).
\end{itemize}
...
\begin{itemize}
  \item Type I error rate under the global null hypothesis ($\alpha \leq 0.025$),
  \item Power under clinically relevant alternatives,
  \item Bias, mean squared error (MSE), and average sample size,
  \item Probability of early stopping for futility or efficacy,
  \item Rate of prior-data conflict detection.
\end{itemize}
...
\textbf{Dynamic $\alpha$ extension.} Place a $\text{Beta}(a_0,b_0)$ hyperprior on $\alpha$ or estimate it by maximising the marginal likelihood. Prior-data conflict is flagged when the posterior mode of $\alpha$ falls below 0.1.
